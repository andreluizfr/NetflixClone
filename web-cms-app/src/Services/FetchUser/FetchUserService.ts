...
export const FetchUserService = () => {
    ...
    if(!accessToken)
        throw new Error('Erro: Token de acesso não encontrado.');

    const queryResult = useQuery<IHttpResponse<User>, IHttpError>(
        ['fetchUser'],
        async () => FetchUserHttpRequest(accessToken),
        {
            enabled: true,
            staleTime: 5 * 1000,
    ...
    useEffect(()=>{
        if (queryResult.isError && queryResult.error) HandleFetchUserQueryError(dispatch);
...
export async function FetchUserHttpRequest (accessToken: string){
