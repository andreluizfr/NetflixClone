...
CREATE TABLE IF NOT EXISTS public.user_info (
    ...
    CONSTRAINT user_info_role_check CHECK (role_id >= 0 AND role_id <= 3),
...
--        index_def TEXT;
--    BEGIN
--        partition_date := to_char(NEW.dh,'YYYY_MM');
...
--            EXECUTE 'CREATE TABLE IF NOT EXISTS ' || current_partition || ' (check (to_char(dh::timestamp::date,''YYYY_MM'') = ''' || to_char(NEW.dh::timestamp::date,'YYYY_MM') || ''')) INHERITS (' || TG_TABLE_NAME || ');'
...
--            END LOOP;
--        END IF;
--        EXECUTE 'INSERT INTO ' || current_partition || ' SELECT(' || TG_TABLE_NAME || ' ' || quote_literal(NEW) || ').* ON CONFLICT DO NOTHING RETURNING cod;';
--        RETURN NULL;
--        EXCEPTION
